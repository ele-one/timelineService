...
# Will create a new directory inside '/' i.e. /timelineService and cd to it as well IN the docker image. Do not confuse with the pwd of local app where dockerfile exist and pwd inside docker image.
WORKDIR timelineService

# Copy package.json first and not do 'Copy . .' because that will take time and npm run will try to run.

# /timelineService is a absolute path, could also have done --> COPY package.json .
COPY package.json /timelineService

RUN npm install

# Copying everything from the local folder where the Dockerfile exist to the docker's '/timelineService'
COPY . /timelineService
